...
const buttonVariants = cva(
  // Changed rounded-md to rounded-full for unique pill shape
  // Added hover:-translate-y-0.5 for a "lift" interaction
  "inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-full text-sm font-medium transition-all duration-200 hover:-translate-y-0.5 disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg:not([class*='size-'])]:size-4 shrink-0 [&_svg]:shrink-0 outline-none focus-visible:border-ring focus-visible:ring-ring/50 focus-visible:ring-[3px] aria-invalid:ring-destructive/20 dark:aria-invalid:ring-destructive/40 aria-invalid:border-destructive",
  {
    variants: {
      variant: {
        ...
        ghost:
          'hover:bg-accent hover:text-accent-foreground dark:hover:bg-accent/50 hover:translate-y-0', // Remove lift for ghost
        link: 'text-primary underline-offset-4 hover:underline hover:translate-y-0',
      },
      size: {
        default: 'h-10 px-6 py-2 has-[>svg]:px-4', // Slightly wider for pill shape
